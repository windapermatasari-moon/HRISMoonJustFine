    ...
    <div className="talenta-shell">
      ...
      <main className="talenta-main">
        ...
        <section className="page">
          ...
          {menu === 'employees' && <Employees data={filteredEmployees} onDelete={deleteEmployee}
...
function Employees({data,onDelete,onExport}:{data:Karyawan[];onDelete:(k:Karyawan)=>void;onExport:()=>void}) {
  return <><Heading title="Semua Karyawan" desc="Master data workforce, jabatan, status, dan payroll." action="＋ Tambah Karyawan"/>
    ...
    <div className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>Nama</th><th>ID</th><th>Jabatan</th><th>Departemen</th><th>Status</th><th>Gaji Pokok</th><th>Aksi</th></tr></thead>
      <tbody>{data.length ? data.map(k=><tr key={k.id}><td><div className="person"><div className="mini-avatar">{k.nama?.charAt(0)||'K'}</div><b>{k.nama}</b></div></td><td>{k.id_karyawan||'-'}</td><td>{k.jabatan||'-'}</td><td>{k.departemen||'-'}</td><td><Status value={k.status_aktif === false ? 'Nonaktif' : 'Aktif'}/></td><td>{money(Number(k.gaji_pokok||0))}</td><td><button className="danger-text" onClick={()=>onDelete(k)}>Hapus</button></td></tr>) : <Empty cols={7}/>}</tbody></table></div></div></>;
}
